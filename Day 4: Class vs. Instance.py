...
https://www.hackerrank.com/challenges/30-class-vs-instance/tutorial
...
If you're failing this challenge, 
...
# Solution
class Person:
    def __init__(self,initialAge): # initialAge is the parameter of the class Person and __init__ method is the constructor of the class
        # Add some more code to run some checks on initialAge
        if initialAge < 0:
...
for i in range(0, t): # for each test case
    age = int(input()) # age is the age of the person
    p = Person(age) # p is the instance of the class Person
    p.amIOld() # call the method amIOld of the instance p
    for j in range(0, 3): # for each year
        p.yearPasses() # call the method yearPasses of the instance p
    p.amIOld() # call the method amIOld of the instance p
