...
from a string and print a custom error message. 
...
Sample Case a contains an integer, so it should not raise an exception when we attempt to convert it 
to an integer. Thus, we print the 3.
...
# remove this peace of sh**t: if __name__ == '__main__': 
# and correct the indentation of the following line:
...
# Explanation:
'''
The input is a string, S, and we need to print its integer value.
If S cannot be converted to an integer, we print Bad String.
'''